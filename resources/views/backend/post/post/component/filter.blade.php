<form action="{{route('post.post.index')}}">
        ...
        <div class="uk-flex uk-flex-middle uk-flex-space-between">
            ...
            <div class="action">
                <div class="uk-flex uk-flex-middle">
                    ...
                    <div class = "ml10">
                        <?php
                            $postCatalogueId = request()->has('post_catalogue_id') ? request('post_catalogue_id') : -1;
                        ?>
                        <select name = "post_catalogue_id" class = "form-control mr10 setupSelect2">
                            <option value="-1" selected="selected">{{__('messages.default.postCatalogue')}}</option>
                            @foreach ($dropdown as $key=>$value)
                                <option {{($postCatalogueId == $key) ? 'selected' : ''}} value="{{$key}}">{{$value}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <a href="{{route('post.post.create')}}" class="btn btn-danger mt5"><i class="fa fa-plus mr5"></i> {{__('messages.post.post.store.create.title')}}</a>
                </div>
            </div>
        </div>
    </div>
